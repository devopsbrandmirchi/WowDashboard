...
const TABS = [
  { id: 'campaigns', label: 'Campaigns', nameColLabel: 'Campaign Name', totalLabel: (n) => `All Campaigns (${n})`, sectionTitle: 'CAMPAIGNS STATISTICS' },
  { id: 'adgroups', label: 'Ad Groups', nameColLabel: 'Ad Group Name', totalLabel: (n) => `All Ad Groups (${n})`, sectionTitle: 'AD GROUPS STATISTICS' },
...
const CHART_METRICS = [
  { key: 'cost', label: 'Cost', fmt: fU, color: '#00809D', axis: 'left' },
...
function computeTotals(rows) {
  const t = { impressions: 0, clicks: 0, cost: 0, purchases: 0 };
  rows.forEach((r) => {
...
function formatDateRange(from, to) {
  ...
  const fmt = (s) => {
    ...
    return `${months[m - 1]} ${String(d || 1).padStart(2, '0')}, ${y}`;
  };
...
const MS_ADS_LOGO = (
  <svg viewBox="0 0 24 24" width="20" height="20">
    <path d="M5 3v16.1l4.5 2.5 8-4.6v-4.3L10 8.5V1L5 3z" fill="#00809D" />
    <path d="M10 8.5v7.1l5.5 3.1 2-1.1v-4.3L10 8.5z" fill="#00B294" opacity=".85" />
  </svg>
);

const MS_ACCENT = '#00809D';

export function MicrosoftAdsReportPage() {
  const { branding, registerExportPdf } = useApp();
  const { filters, dateRange, batchUpdateFilters, fetchData, loading, error, campaigns, adGroups, placements, countryData, productData, showsData, days, kpis, dailyTrends } = useMicrosoftAdsReportData();
  ...
  const [kpiSelected, setKpiSelected] = useState(() => {
    try {
      const saved = localStorage.getItem('microsoft_ads_kpi_selection');
  ...
  const countMap = { campaigns: campaigns.length, adgroups: adGroups.length, placements: placements.length, country: countryData.length, product: productData.length, shows: showsData.length, day: days.length };

  const handleApply = useCallback(() => {
    TABS.forEach((t) => setPg((prev) => ({ ...prev, [t.id]: 1 })));
    batchUpdateFilters({ campaignSearch });
    setTimeout(() => fetchData(), 0);
  }, [batchUpdateFilters, fetchData, campaignSearch]);
  ...
  const handleKpiSelect = useCallback((slotIdx, newKey) => {
    setKpiSelected((prev) => {
      ...
      next[slotIdx] = newKey;
      try { localStorage.setItem('microsoft_ads_kpi_selection', JSON.stringify(next)); } catch {}
      return next;
    });
  ...
  const handleCSV = useCallback(() => {
    ...
    exportCSV(csvCols, data, `microsoft-ads-report-${activeTab}.csv`);
  }, [activeTab, hiddenCols, tabDataMap]);
  ...
  exportPdfRef.current = () => {
    ...
    exportReportPdf({
      reportTitle: 'Microsoft Advertising Metrics',
      dateRangeText,
      kpis: kpiList,
      tableHeaders: headers,
      tableRows: rows,
      branding,
      filename: `microsoft-ads-report-${activeTab}`,
    });
  };
  ...
  return (
    <div className="page-section active" id="page-microsoft-ads-report">
      <div className="page-content">
        <div className="page-title-bar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h2 style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--navy)' }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36 }}>{MS_ADS_LOGO}</span>
              Bing / Microsoft Ads
            </h2>
            <p style={{ marginTop: 4, fontSize: 13, color: 'var(--text-muted)' }}>
              {formatDateRange(dateRange?.from || filters.dateFrom, dateRange?.to || filters.dateTo) || 'Select date range'}
            </p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            />
            <button type="button" className="btn btn-primary btn-sm" onClick={handleDownload} style={{ background: MS_ACCENT, borderColor: MS_ACCENT }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ marginRight: 6, verticalAlign: '-2px' }}><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3" /></svg>
              Download
            </button>
          </div>
        </div>

        <div className="gads-filter-bar" id="microsoft-ads-filter-bar">
          <div className="gads-filter-row">
            <div className="gads-filter-group gads-fg-sm">
              <label>Campaign</label>
              <input type="text" placeholder="Search by campaigns..." className="gads-search-input" value={campaignSearch} onChange={(e) => setCampaignSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleApply()} />
            </div>
        ...
        {error && (
          <div style={{ padding: '16px 20px', background: 'var(--danger-bg)', color: 'var(--danger)', borderRadius: 'var(--radius-md)', margin: '0 0 16px', fontSize: 13, display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <button type="button" className="btn btn-primary btn-sm" onClick={handleRetry} style={{ background: MS_ACCENT, borderColor: MS_ACCENT }}>Retry</button>
          </div>
        )}
        ...
        <div className="gads-kpi-section">
          ...
          {!kpiCollapsed && (
            <div className="kpi-grid-6" id="microsoft-ads-kpi-grid">
        ...
        <div className="gads-tabs-container">
          <div className="gads-tabs-row">
            <div className="gads-tabs">
              {TABS.map((tab) => (
                <button key={tab.id} type="button" className={`gads-tab ${activeTab === tab.id ? 'active' : ''}`} onClick={() => setActiveTab(tab.id)} style={activeTab === tab.id ? { color: MS_ACCENT, borderBottomColor: MS_ACCENT } : undefined}>
        ...
        <div id="microsoft-ads-tab-content">
