...
export function useMicrosoftAdsReportData() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [campaignRows, placementRows, refRows] = await Promise.all([
        fetchAllRows(() => supabase.from('microsoft_campaigns_ad_group').select('*')),
        fetchAllRows(() => supabase.from('microsoft_campaigns_placement').select('*')).catch(() => []),
        fetchAllRows(() => supabase.from('microsoft_campaigns_reference_data').select('*')).catch(() => []),
      ]);
      ...
      console.error('Microsoft Ads fetch error:', err);
